...
def create_coolify(server_ip, username, base_path, logfile):
    ...
    key = paramiko.RSAKey(filename='id_rsa')
    ssh.connect(server_ip, username=username, pkey=key)
    coolify_path = f"{base_path}/core/coolify"
    commands = [
        f'mkdir -p {coolify_path}',
        f'cd {coolify_path}',
        f'curl -fsSL https://cdn.coollabs.io/coolify/install.sh | bash',
    ]
    log_output(f"Creating coolify install at: {coolify_path}", logfile)
    for command in commands:
        log_output(f"Executing: {command}", logfile)
        stdin, stdout, stderr = ssh.exec_command(command)
        stdout.channel.recv_exit_status()
        log_output(f"STDOUT: {stdout.read().decode()}", logfile)
        log_output(f"STDERR: {stderr.read().decode()}", logfile)
    log_output("Coolify created on server.", logfile)
...
if __name__ == "__main__":
    ...
    create_coolify(server_ip, username, base_path, logfile)
