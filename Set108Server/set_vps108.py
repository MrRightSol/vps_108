...
def create_user(server_ip, root_username, new_username, new_password, logfile):
    ssh = get_my_ssh_connection(server_ip, root_username)
...
def get_my_ssh_connection(server_ip, new_user, key_filename='id_rsa'):
    ssh = paramiko.SSHClient()
    ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    key = paramiko.RSAKey(filename=key_filename)
...
http:
  middlewares:
    ...
    user-auth:
      basicAuth:
        users:
          - "marktraf:$apr1$2D5R1pqp$VsgT5Oeek.vu3.UI9TZnv0"
...
if __name__ == "__main__":
    ...
    ssh = get_my_ssh_connection(server_ip, new_user)
